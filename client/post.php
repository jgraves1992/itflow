...
if (isset($_GET['stripe_save_card'])) {
    ...
    try {
        ...
        $pm_type = escapeSql($payment_method_details->type);

        if ($pm_type === 'us_bank_account') {
            $bank_name = escapeSql($payment_method_details->us_bank_account->bank_name);
            $last4     = escapeSql($payment_method_details->us_bank_account->last4);
