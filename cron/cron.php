...
while ($row = mysqli_fetch_assoc($sql_recurring_payments)) {
    ...
    if ($recurring_payment_saved_payment_id) {
        ...
        $expense_percentage_fee_ach = floatval($saved_payment['payment_provider_expense_percentage_fee_ach']);
        $expense_flat_fee_ach = floatval($saved_payment['payment_provider_expense_flat_fee_ach']);
        ...
        if ($provider_name === "Stripe") {
            if ($provider_private_key && $stripe_customer_id && $stripe_payment_method_id) {
                ...
                if (in_array($payment_intent->status, ['succeeded', 'processing']) && intval($balance_to_pay) == intval($pi_amount_paid)) {
                    ...
                    // EXPENSE: Stripe gateway fee as an expense (if configured) — ACH and card have different fee structures
